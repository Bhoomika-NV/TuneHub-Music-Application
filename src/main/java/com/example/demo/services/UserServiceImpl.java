...
public class UserServiceImpl implements UsersServices {
	...
	public boolean validateUser(String email, String password) { // task 3 checking, email and password matches
		...
		String db_password = user.getPassword(); // then repository returns an object of entity within which all
													// attribues are presennt.
		if (db_password.equals(password) == true) // And also we have pasword provided by the user while loggin ,
		{ // using the entity object we have to get the password by
			// entitYObjectreference.getPassword();
			return true; // since the user entered password and Database password are string we will use
							// equals() to compare
